...
const groupByLengths = (arr) =>
  arr.reduce(
    (sum, i) => ({ ...sum, [i.length]: [...(sum[i.length] || []), i] }),
    {}
  );
...
const hasAll =
  (...match) =>
  (str) =>
    match.every((k) => k.split("").every((j) => str.includes(j)));
...
const findNumbers = (arr) => {
  ...
  add(
    "2",
    byLength[5].find((d) => d.includes(f))
  );
  add("5", unknown[0]); // woho final

  return Object.entries(known).reduce(
    (sum, [k, v]) => ({ ...sum, [parseSegment(v)]: k }),
    {}
  );
};
