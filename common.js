const numbers =
    (separator = ",") =>
        (input) =>
            input.split(separator).toNumber();
...
const count = (nr) => (line) =>
    line.reduce((sum, i) => sum + (i === nr ? 1 : 0), 0);
...
const extentLines = (points) => {
    const size = points.reduce(
        ({ top, left, width, height }, { x1, y1, x2, y2 }) => {
            return {
...
const aStar = (startPos, getPossibleSteps, isEnd, getCost) => {
    ...
    while (toTry.length > 0) {
        const { pos, cost, path = [] } = toTry.pop();

        if (isEnd(pos)) {
            return { cost, path };
        }

        getPossibleSteps(pos).forEach(({ idx, ...p }) => {
            if (!walked.has(idx)) {
                walked.add(idx);
...
const getClosest =
    ({ width, height, top = 0, left = 0 }, diagonal = false) =>
        ({ x, y }) => {
            const pos = [];
